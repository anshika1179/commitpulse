...
function calculateRadarMetrics(stats: StreakStats, calendar: ContributionCalendar, seed: string): number[] {
  // 1. Consistency: current streak vs longest
  const consistency = stats.longestStreak > 0 ? Math.min(1, stats.currentStreak / stats.longestStreak) : 0;
